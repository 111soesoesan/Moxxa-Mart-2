# Moxxa Mart - AI Assistant Guidelines (Claude.md)
...
## 5. Key Integrations
- **Payments**: System uses structured payment methods (stored in `payment_methods` table). Legacy `payment_info` JSONB fields have been deprecated and should not be used.
- **Product Management**: Supports variable products with attributes and variations. Modifying products utilizes `categories.ts`, `attributes.ts`, and `products.ts` server actions.
...
1. **System Understanding**: Always review this `Claude.md` file and any relevant `supabase/docs/` files before beginning a task.
